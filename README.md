...
- **Интерактивность.** `set({interact:…})` навешивает на ячейку контекстное меню по правой
  кнопке и подсказку по наведению: нативное меню Zabbix для item/host/trigger (Последние
  данные, График, Триггеры, скрипты…) и хинтбоксы, либо своя подсказка и свои ссылки —
  задаётся как данные, никакого кода на странице.
- **Чарт-хинты.** Подсказка по наведению может быть живым графиком истории с вкладками
  (`hint.history`) — рисуется на Canvas по сырой истории за интервал виджета (поле
  «Time period»), с прогрессивной загрузкой страницами до 500 значений (сначала свежие,
  каждая следующая — от последнего полученного; устойчиво к тротлингу).
