...
- **Интерактивность:** ячейка может нести **контекстное меню по правой кнопке** и
  **всплывающую подсказку по наведению**. Наведи их на айтем/хост/триггер — получишь
  штатное меню Zabbix (Последние данные, График, Триггеры, скрипты…) и попапы; либо
  собери свою подсказку и свои ссылки сам — всё задаётся как данные, никакой
  дополнительный код на странице не исполняется.
- **Чарт-хинты:** подсказка по наведению может быть **живым графиком истории с вкладками**
  для любых айтемов (`hint.history`) — рисуется на Canvas по сырой истории, за интервал
  виджета, с прогрессивной загрузкой страницами до 500 значений (сначала свежие, каждая
  следующая — от последнего полученного; устойчиво к тротлингу). У Zabbix нет нативного
  аналога внутри диаграммы.
...
| **Time period** | интервал для чарт-хинтов (`hint.history`) — дашборд, кастомный диапазон или другой виджет |
...
- **`patch`** `object` — любое подмножество:
  ...
  | `interact` | object | Меню по правой кнопке и/или подсказка по наведению для ячейки (см. **interact** ниже). `{}` — очистить. |
...
**`interact`** (значение `set({interact})`): заставляет ячейку реагировать на мышь. Это
просто декларация — виджет превращает её в штатное поведение Zabbix, поэтому **никакой
пользовательский код на странице не исполняется** (модель песочницы сохранена). Как и
другие поля патча, оно **липкое**; передай `{}`, чтобы очистить.
...
- **`hint`** — подсказка по наведению. `{ html }` или `{ text }` рисуют твой контент (из
  payload, без похода на сервер). `{ preload: { type, data } }` показывает **нативный
  хинтбокс, отрисованный сервером** — виджет сам подтягивает его HTML: `type: 'eventlist'`
  (список проблем/событий триггера) или `type: 'eventactions'` (история действий по
  событию), либо `action: '<hintbox.*>'` для любого hintbox-эндпоинта напрямую. Для текущих
  проблем триггера есть готовая структура на каждом триггере: `hint: { preload: trigger.event_hint }`
  (присутствует, только пока у триггера есть открытая проблема).
  `{ history: { '<Вкладка>': [itemid, …], … } }` вместо этого показывает **живой график
  истории** — по вкладке на метку, каждая рисует свои айтемы за **Time period** виджета
  (настраивается в форме). Рисуем на Canvas по сырой истории, тянем страницами до 500
  значений (сначала свежие, каждая следующая — от последнего полученного), поэтому тяжёлый
  или тротлящийся запрос отрисовывается прогрессивно. Пример:
  `hint: { history: { 'CPU': [it['cpu'].itemid], 'Net': [it['in'].itemid, it['out'].itemid] } }`.
- **`menu`** — контекстное меню по **правой кнопке**, нативное для Zabbix:
  `{ type: 'item', itemid }`, `{ type: 'host', hostid }` или `{ type: 'trigger', triggerid }`.
  Получаешь то же меню, что и везде в Zabbix (Последние данные, График, Триггеры,
  скрипты хоста…), которое само подтягивает данные по требованию.
- **`links`** — своё меню по правой кнопке целиком: `[{ label, url, target }]`. Имеет
  приоритет над `menu` на той же ячейке. Годится для переходов на любой URL.

Id берутся из внедрённых данных: `item.itemid`, `host.hostid`, `trigger.triggerid`.
