...
- **`fn`** `(cell) => boolean` — предикат; `cell` — простой дескриптор `{ id, label, bbox, neighbors }` (не handle).
...
- **`patch`** `object` — любое подмножество:
  ...
  | `flow` | number | Бегущий пунктир по линиям ячейки: знак = направление, модуль = скорость, `0`/`false` = стоп. |
